...
import static com.stanfy.helium.handler.codegen.objectivec.builder.ObjCPropertyNameTransformer.KEYWORDS
...
class ObjCPropertyNameTransformerSpec extends Specification{
  ...
  def "should not allow to generate property names with reserved keywords Complex check"() {
    given:
    def convertedProperties = KEYWORDS.collectAll { s -> propertyNameTransformer.propertyNameFrom(s)}
    expect:
